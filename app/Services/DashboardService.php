        ...
        $activeTasks = Ticket::with(['department:id,name,code', 'resolution.category:id,name,infrastructure_type'])
            ...
            ->map(function ($t) {
                ...
                return [
                    ...
                    'category_name' => $t->resolution?->category ? $t->resolution->category->name : '-',
                    'infrastructure_type' => $t->resolution?->category ? $t->resolution->category->infrastructure_type : '-',
                    'network_type' => $t->resolution?->category ? $t->resolution->category->infrastructure_type : '-',
                    'priority' => $t->priority,
        ...
        $recentFeedbacks = Ticket::with(['department:id,name,code', 'reporter:id,name', 'feedback'])
            ...
            ->map(function ($t) {
                return [
                    ...
                    'rating' => (int) ($t->feedback?->rating ?? 0),
                    'feedback_comment' => $t->feedback?->feedback_comment ?? '-',
                    'rated_at' => $t->feedback?->created_at ? $t->feedback->created_at->format('d M Y, H:i') : ($t->updated_at ? $t->updated_at->format('d M Y, H:i') : '-'),
                    'rated_at_diff' => $t->feedback?->created_at ? $t->feedback->created_at->diffForHumans() : ($t->updated_at ? $t->updated_at->diffForHumans() : '-'),
                ];
            })
            ->values()
            ->toArray();
        ...
        return $query->latest()
            ...
            ->map(function ($ticket) {
                ...
                return [
                    ...
                    'category_name' => $ticket->resolution?->category ? $ticket->resolution->category->name : '-',
                    'technicians_label' => !empty($technicianNames) ? implode(', ', $technicianNames) : 'Belum ditugaskan',
                    'infrastructure_type' => $ticket->resolution?->category ? $ticket->resolution->category->infrastructure_type : '-',
                    'network_type' => $ticket->resolution?->category ? $ticket->resolution->category->infrastructure_type : '-',
